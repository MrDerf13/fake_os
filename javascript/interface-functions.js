...
const openApp = (target) => {
    ...
    closeWindow.addEventListener("click", (event)=>closeApp(cont, intervalTracker));
...
const loadDesktop = () => {
    ...
    for (let i = desktopAppsArr.length-1; i>=0 ; --i){
        desktop.insertAdjacentHTML("afterbegin", `<div class="app"}><img src=${desktopAppsArr[i].desktopImg} alt=${desktopAppsArr[i].name} id=${desktopAppsArr[i].name}><label for=${desktopAppsArr[i].name} id=${desktopAppsArr[i].name}>${desktopAppsArr[i].display}</label></div>`)
}
}
...
const loadTaskbar = () => {
    ...
    for (let i = taskbarArr.length-1; i>=0 ; --i){
        desktop.insertAdjacentHTML("afterbegin", `<div class="app"><img src=${taskbarArr[i].taskbarImg} alt=${taskbarArr[i].name}  id=${taskbarArr[i].name}></div>`)
}
}
...
const unpinFromTaskbar = (app, id) =>{
    ...
    if (appIndex>-1){
        const prompt = confirm(`Would you like to unpin ${app.display} from the task bar?`)
        if (prompt){
